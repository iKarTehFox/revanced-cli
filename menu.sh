...
choose_file_path() {
    ...
    # Open file picker dialog using zenity (GTK), fallback to manual input
    local file_path
    if command -v zenity &>/dev/null; then
        file_path=$(zenity --file-selection --title="Select $description" --file-filter="*.$extension" 2>/dev/null)
    else
        read -p "Enter path to $description (*.$extension): " file_path
    fi
